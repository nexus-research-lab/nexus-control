...
- Runtime、Relay User 与 Relay Node 使用独立 Principal audience。Node 只由有效浏览器 Session 显式注册，绑定组织、真人所有者和最多 32 个自己发布的 Agent；Control 仅存凭据哈希，机器凭据只能换取 15 分钟 Node 令牌，不能访问真人 API。
